...
\begin{frame}{Craps}
\begin{itemize}
\item Each player in either \alert{pass betters}
or \alert{don't pass betters}
\item Pass betters roll two dice
\item If $7$ or $12$ the sum (a \alert{natural}) then pass betters win
\item If $2$, $3$, or $12$ the sum (a \alert{craps})
then don't pass betters win
\item If $4,5,6,8,9,10$ the sum then this number called the \alert{point}
\item Pass betters roll dice until sum is either $7$ or the point
\item If point occurs first, then pass betters win (\alert{point is made})
\item If $7$ occurs first, then don't pass betters win
...
\begin{frame}
\begin{itemize}
\item Want to calculate expected proceeds for pass betters
...
\item $3$ ways to roll $4$ so $P\left(4\right)=\frac{4}{36}$
...
=\frac{6}{36}\cdot\frac{3}{9}=\frac{1}{18}$
\end{itemize}
\end{frame}
...
of all outcomes
\item Assuming that pass betters and
don't pass betters each bet $100$,
