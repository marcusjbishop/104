...
\item Everyone has \alert{genotype} consisting
of two elements of $\left\{c,C\right\}$,
possibly the same
\item So four possible genotypes $cc,cC,Cc,CC$
\item Individuals with genotype $cc$ have Cystic Fibrosis,
but individuals with $cC,Cc,CC$ do not
\item In other words, anyone with a $C$ does \alert{not} have Cystic Fibrosis
...
with equal probability, and one gene of Mother's genotype
