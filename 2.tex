...
\item \alert{Your responsibility} to determine which
...
\item However, please discuss homework collection with Josh, not with me
\end{itemize}
\end{frame}
...
\item Everyone has \alert{gene pair}
with regard to Cystic Fibrosis gene
\item \alert{Gene Pair} a sequence of two elements of
$\left\{c,C\right\}$, possibly the same
\item So four possible gene pairs $cc,cC,Cc,CC$
\item Individuals with gene pair $cc$ \alert{have} Cystic Fibrosis,
...
\item Individuals with gene pair $cC$ or $Cc$ called \alert{carriers}
\end{itemize}
\end{frame}

\begin{frame}
\begin{itemize}
\item First letter listed in gene pair
inherited from father, second from mother
\begin{example}
An individual with gene pair $cC$ inherited
$c$ from father and $C$ from mother
\end{example}
\item Child receives one gene from Father's gene pair,
both with probability $\frac{1}{2}$
\item Similarly child receives one gene from Mother's gene pair,
both with probability $\frac{1}{2}$
\end{itemize}
\end{frame}
...
\begin{frame}{Example}
\begin{itemize}
\item Suppose Father has gene pair~$cC$ and mother has~$CC$
...
\item Entries give possible child gene pairs, each equally likely
\item Note that \alert{none} of possible child gene pairs is $cc$
...
\begin{frame}{Example}
\begin{itemize}
\item Suppose Father and Mother both have gene pair~$cC$
\item Calculate probability that child will have Cystic Fibrosis
\item[]
\[\begin{array}{c|cc}
&c&C\\\hline
...
C&Cc&CC
\end{array}\]
\item One of four possible gene pairs is $cc$
