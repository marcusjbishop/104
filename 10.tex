...
cards to exchange
\item Then game is over, player wins amount specified in table
\item Game can be played in under 30 seconds,
so players generally play many times
\item Thus expectation helpful
...
\item A \alert{strategy} a systematic plan dictating
...
multiply payoff by probability and add
...
\item $50-1$ the expected proceeds if she discards no cards
...
\end{example}
\begin{itemize}
\item Similar analysis should be performed for each
of $\ncr{52}{5}$ hands
\item So we know what action player should take in
every possible situation
\item Assuming player follows strategy, can calculate
overall expected outcome of game
\item Namely, multiply expectation by
$\frac{1}{\ncr{52}{5}}$ and add
\end{itemize}
\end{frame}
...
\begin{frame}
\begin{itemize}
\item One such calculation gives $1.0239>0$(!)
...
\item Impractical to ask player to look up number
of cards to discard from long list
\end{itemize}
\end{frame}
