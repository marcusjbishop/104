...
\begin{frame}
\begin{itemize}
\begin{example} $2\hs,2\ds,Q\cs,Q\s,10\ds$ has two pairs\end{example}
\item If another player has two pairs, tie broken
by comparing highest ranking pairs
\begin{example} $K\s,K\hs,7\ds,7\s,A\s$ has two pairs,
and beats hand above, since $K$ higher than $Q$\end{example}
\item If two players have
highest ranking pair of same rank, then tie broken
by comparing lowest ranking pairs
\begin{example} $K\cs,K\ds,J\hs,J\cs,3\hs$ has two pairs,
...
\item Analogous to factorials in denominator of number
of anagrams
\begin{example}
\begin{itemize}
